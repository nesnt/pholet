...
export const Navbar: React.FC<NavbarProps> = ({
  ...
  return (
    <>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20 gap-4">
            ...
            <div className="flex md:hidden items-center gap-2">
              <button
                onClick={() => onViewChange('design-spec')}
      ...
      <nav className="md:hidden fixed bottom-0 left-0 right-0 z-40 bg-[#622B14] border-t border-[#995F2F]/40 shadow-lg px-2 py-1.5 flex items-center justify-around">
        <button
          onClick={() => onViewChange('feed')}
          className={`flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl text-xs font-medium transition-colors ${
            currentView === 'feed' ? 'text-[#E4D6A9]' : 'text-[#978F66]'
          }`}
        >
        ...
        <button
          onClick={() => onViewChange('design-spec')}
          className={`flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl text-xs font-medium transition-colors ${
            currentView === 'design-spec' ? 'text-[#E4D6A9]' : 'text-[#978F66]'
          }`}
        >
          <BookOpen className="w-5 h-5" />
          <span>Desain</span>
        ...
        <button
          ...
          className={`flex flex-col items-center gap-0.5 px-3 py-1 rounded-xl text-xs font-medium transition-colors ${
            currentView === 'profile' ? 'text-[#E4D6A9]' : 'text-[#978F66]'
          }`}
        >
